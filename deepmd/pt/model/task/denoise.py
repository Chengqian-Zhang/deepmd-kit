...
class DenoiseNet(Fitting):
    def __init__(
        ...
        **kwargs,
        ...
        """Construct a direct force ener fitting net.
        ...
        self.filter_layers_virial = NetworkCollection(
            1 if not self.mixed_types else 0,
            self.ntypes,
            network_type="fitting_network",
            networks=[
    ...
    def serialize(self) -> dict:
        ...
        return {
            ...
            "virial_nets": self.filter_layers_virial.serialize(),
    ...
    def deserialize(self) -> "DenoiseNet":
        ...
        virial_nets = data.pop("virial_nets")
        force_nets = data.pop("force_nets")
        obj = cls(**data)
        for kk in variables.keys():
            obj[kk] = to_torch_tensor(variables[kk])
        obj.filter_layers_virial = NetworkCollection.deserialize(virial_nets)
    ...
    def forward(
        ...
        if self.mixed_types:
            ...
            # virial
            for type_i, ll in enumerate(self.filter_layers_virial.networks):
                mask = (atype == type_i).unsqueeze(-1)
                mask = torch.tile(mask, (1, 1, 1))
                atom_virial = ll(xx)
                atom_virial = torch.where(mask, atom_virial, 0.0)
                outs = (
                    outs + atom_virial
        ...
        return {
            "virial": outs.to(env.GLOBAL_PT_FLOAT_PRECISION),
            "force": vec_out,
        }