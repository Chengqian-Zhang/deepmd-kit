...
class EnergyFittingNetDirect(Fitting):
    ...
    def deserialize(self) -> "EnergyFittingNetDirect":
        ...
        energy_nets = data.pop("energy_nets")
    ...
    def forward(
        ...
        if self.mixed_types:
            ...
            # energy
            atom_energy = self.filter_layers.networks[0](xx)
            if xx_zeros is not None:
                atom_energy -= self.filter_layers.networks[0](xx_zeros)
            outs = (
            ...
            for type_i, ll in enumerate(self.filter_layers.networks):
                ...
                if xx_zeros is not None:
                    ...
                    if not (
                        ...
                        atom_energy -= ll(xx_zeros)
                atom_energy = atom_energy + self.bias_atom_e[type_i].to(self.prec)
                atom_energy = torch.where(mask, atom_energy, 0.0)
                outs = (
                    outs + atom_energy
                )  # Shape is [nframes, natoms[0], 1]
        # nf x nloc
